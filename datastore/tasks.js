...
cli
  .demand(1)
  .command(`new <description>`, `Adds a task with a description <description>.`, {}, (opts) => {
    addTask(opts.description, noop);
  })
  .command(`done <taskId>`, `Marks the specified task as done.`, {}, (opts) => {
    markDone(opts.taskId, noop);
  })
  .command(`list`, `Lists all tasks ordered by creation time.`, {}, (opts) => {
    listTasks(noop);
  })
  .command(`delete <taskId>`, `Deletes a task.`, {}, (opts) => {
    deleteTask(opts.taskId, noop);
  })
